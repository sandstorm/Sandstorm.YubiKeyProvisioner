...
# Disable SmartCard UI otherwise we will get a pairing request every time we
# insert a YubiKey
currentUser=`who | grep "console" | cut -d" " -f1`
sudo su - "$currentUser" -c "/usr/sbin/sc_auth pairing_ui -s disable"


green_echo "STEP 1: installing/updating YubiKey management tools"
brew reinstall ykman
brew reinstall yubico-piv-tool

echo ""
green_echo "STEP 2: removing yubikey-agent"
...
echo ""
green_echo "STEP 3: installing yubikey-agent sandstorm fork"
